# Copyright 2019 ADHOC SA
# License AGPL-3.0 or later (http://www.gnu.org/licenses/agpl).
from psycopg2.extensions import AsIs

from odoo import fields, models, tools


class MisCashFlow(models.Model):
    _name = "mis.cash_flow"
    _description = "MIS Cash Flow"
    _auto = False

    line_type = fields.Selection(
        [("forecast_line", "Forecast Line"), ("move_line", "Journal Item")],
        index=True,
        readonly=True,
    )
    name = fields.Char(readonly=True)
    account_id = fields.Many2one(
        comodel_name="account.account",
        string="Account",
        index=True,
        readonly=True,
    )
    partner_id = fields.Many2one(
        comodel_name="res.partner",
        string="Partner",
        readonly=True,
    )
    move_line_id = fields.Many2one(
        comodel_name="account.move.line",
        string="Journal Item",
        readonly=True,
    )
    company_id = fields.Many2one(
        comodel_name="res.company",
        string="Company",
        readonly=True,
        index=True,
    )
    credit = fields.Float(readonly=True)
    debit = fields.Float(readonly=True)
    date = fields.Date(readonly=True, index=True)
    reconciled = fields.Boolean(readonly=True)
    full_reconcile_id = fields.Many2one(
        "account.full.reconcile",
        string="Matching Number",
        readonly=True,
        index=True,
    )
    account_type = fields.Selection(selection="_selection_account_type", readonly=True)
    # Legacy coarse classification (pre-19.0 account.account.user_type_id.type).
    # 19.0 replaced it with the granular account_type; we map it back so
    # cash-flow reports authored before 19.0 — which filter on
    # account_internal_type (e.g. ('account_internal_type','=','liquidity')) —
    # keep computing without rewriting their saved KPI expressions.
    account_internal_type = fields.Selection(
        [
            ("receivable", "Receivable"),
            ("payable", "Payable"),
            ("liquidity", "Liquidity"),
            ("other", "Regular"),
        ],
        readonly=True,
    )
    parent_state = fields.Selection(selection="_selection_parent_state")

    def _selection_parent_state(self):
        return self.env["account.move"].fields_get(allfields=["state"])["state"][
            "selection"
        ]

    def _selection_account_type(self):
        return self.env["account.account"].fields_get(allfields=["account_type"])[
            "account_type"
        ]["selection"]

    def init(self):
        query = """
            SELECT
                -- we use negative id to avoid duplicates and we don't use
                -- ROW_NUMBER() because the performance was very poor
                -aml.id as id,
                'move_line' as line_type,
                aml.id as move_line_id,
                aml.account_id as account_id,
                acc.account_type as account_type,
                CASE acc.account_type
                    WHEN 'asset_receivable' THEN 'receivable'
                    WHEN 'liability_payable' THEN 'payable'
                    WHEN 'asset_cash' THEN 'liquidity'
                    ELSE 'other'
                END AS account_internal_type,
                CASE
                    WHEN aml.amount_residual > 0
                    THEN aml.amount_residual
                    ELSE 0.0
                END AS debit,
                CASE
                    WHEN aml.amount_residual < 0
                    THEN -aml.amount_residual
                    ELSE 0.0
                END AS credit,
                aml.reconciled as reconciled,
                aml.full_reconcile_id as full_reconcile_id,
                aml.partner_id as partner_id,
                aml.company_id as company_id,
                aml.name as name,
                aml.parent_state as parent_state,
                COALESCE(aml.date_maturity, aml.date) as date
            FROM account_move_line as aml
            JOIN account_account as acc ON acc.id = aml.account_id
            WHERE aml.parent_state != 'cancel'
            UNION ALL
            SELECT
                fl.id as id,
                'forecast_line' as line_type,
                NULL as move_line_id,
                fl.account_id as account_id,
                acc.account_type as account_type,
                CASE acc.account_type
                    WHEN 'asset_receivable' THEN 'receivable'
                    WHEN 'liability_payable' THEN 'payable'
                    WHEN 'asset_cash' THEN 'liquidity'
                    ELSE 'other'
                END AS account_internal_type,
                CASE
                    WHEN fl.balance > 0
                    THEN fl.balance
                    ELSE 0.0
                END AS debit,
                CASE
                    WHEN fl.balance < 0
                    THEN -fl.balance
                    ELSE 0.0
                END AS credit,
                NULL as reconciled,
                NULL as full_reconcile_id,
                fl.partner_id as partner_id,
                fl.company_id as company_id,
                fl.name as name,
                'posted' as parent_state,
                fl.date as date
            FROM mis_cash_flow_forecast_line as fl
            JOIN account_account as acc ON acc.id = fl.account_id
        """
        tools.drop_view_if_exists(self.env.cr, self._table)
        self.env.cr.execute(
            "CREATE OR REPLACE VIEW %s AS (%s)", (AsIs(self._table), AsIs(query))
        )

    def action_open_related_line(self):
        self.ensure_one()
        if self.line_type == "move_line":
            return self.move_line_id.get_formview_action()
        else:
            return (
                self.env["mis.cash_flow.forecast_line"]
                .browse(self.id)
                .get_formview_action()
            )
