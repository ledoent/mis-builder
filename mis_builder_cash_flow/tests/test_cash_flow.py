...
class TestCashFlow(TransactionCase):
    ...
    def test_report_instance(self):
        ...
        move.action_post()
        # Ensure the stored ``parent_state`` of the move lines is written to the
        # database, otherwise the ``mis.cash_flow`` SQL view (which reads
        # ``account_move_line.parent_state`` directly) and the posted-only
        # target-move filter would still see the pre-posting ``draft`` value.
        self.env.flush_all()
        self.check_matrix(
            args=[
