...
class MisReportInstancePeriod(models.Model):
    ...
    def _get_additional_move_line_filter(self):
        ...
        if self.company_id:
            # Filter move lines to this period's specific company.
    ...
    def _get_additional_query_filter(self, query):
        ...
        if company_field := query.sudo().company_field_id:
            if self.company_id:
                # Period filtered to a specific company
...
class MisReportInstance(models.Model):
    ...
    def action_generate_company_columns(self):
        """Generate one period/column per company in company_ids.

        Each column uses the same date settings but is filtered to a single
        company, plus an optional Total column that sums them.
        """
        self.ensure_one()
        ...
        for company in self.company_ids:
            ...
            seq += 10
        # Create a Total column that sums all company columns
    ...
    def _compute_matrix(self):
        ...
        # Detect if companies are shown as columns (each period has company_id)
